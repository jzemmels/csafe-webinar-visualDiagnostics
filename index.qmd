...
Today, we will discuss an image-analysis algorithm called the Automatic Cartridge Evidence Scoring or "ACES" algorithm that compares cartridge case evidence.
...
- 0.80% overall error rate (26 out of 3,268, 746 inconclusives)

- 1.01% false positive rate (22 out of 2,178 comparisons, 735 inconclusives)

- 0.37% false negative rate (4 out of 1,090 comparisons, 11 inconclusives)
...
- Predicts match probability for a cartridge case pair using trained statistical model
...
The final result is a predicted probability that two cartridge cases match based on a trained statistical model.
...
At the bottom of the slide, you can see the distributions of these features based on the comparisons in our training data. The orange distributions represent the matching comparisons while the gray distributions represent non-matching comparisons.
You'll note that the matching and non-matching distributions have some partial overlap between in these plots.
I show these to demonstrate that these 7 features are successful, to varying degrees, at distinguishing between matching and non-matching comparisons, meaning we can use them in a classification algorithm to classify matching and non-matching pairs.
...
The red points again represent cells that don't agree.
The key finding from this visual is that the estimated registration in scan K013sA1 are almost the exact opposite of the registration in scan K013sA2.
...
The takeaway from this plot is again that these features distingusih between matching and non-matching comparisons.
...
So given a cartridge case pair, this logistic regression model will classify that pair a match or a non-match based on an predicted match probability.
The process of training a model involves selecting model parameters that optimize some criteria.
For example, we consider two different optimization criteria: one where we select the model that minimizes the overall classification error rate and another that balances the false positive and false negative error rates.
...
We are interested not only in the false positive and false negative error rates, but we also want to consider the predicted probabilities of the matching and non-matching comparisons since this actually what an examiner would use during their examination.
...
| Source | False Pos (%) | False Neg (%) | Overall Error (%) |
| ------ | -------------: | -------------: | -----------: |
| ACES, Minimum Error | 0.07 | 7.66 | 0.59 |
| ACES, Balanced FP/FN | 1.93 | 4.25 | 2.09 |
| Ames I | 1.01 | 0.37 | 0.80 |
| Ames I, Inconcl. as errors| 34.76 | 1.37 | 23.62 |

. . .

- We compare *pair*wise (1 to 1), Ames I [@baldwin] compared quartets (3 to 1)
...
The table on this slide summarizes the classification error rates for the two trained models based on the test data.
In the first row, we can see that the model selected to minimize the overall error rate has a low error rate of 0.59% which consists of a false positive error rate of 0.07% and false negative error rate of 7.66%.
In second row, the model selected to balance the false palse positive and false negative error rates has a larger overall error rate of 2.09%, but this consists of a false positive and false negative error rates that are much closer to each other.

The reason that I compare these two models to each other is to highlight the fact that two different criteria lead to considerably different results.
...
Sp there are many criteria we could use, and I wanted to emphasize that these could have radically different outcomes.

For comparison, I've repeated the results reported in the Baldwin study.
We see that the first model actually has a lower overall error rate, although this is at the expense of a much larger false negative error rate.

**NEXT**

Now, a few notes I wanted to make here: the results here are somewhat difficult to compare directly to the Baldwin results since we compare every pair of cartridge cases whereas Baldwin compared a groupd of 3 known-match cartridge cases to 1 unknown source cartrdige case.
...
You can see that there are around 3000 matching comparisons and 42,0000 non-matching comparisons.
This is not at all unique to cartridge case comparisons, but I wanted to point this out to emphasize that if we get more out of erring on the side of non-matching classifications than matching classifications.
For example, you can see this represented in the first model that a very low false positive rate but a large false negative rate.
This model seemingly has a preference for making non-match classifications.

**NEXT**

Finally, some of you may be looking at these results and think "why does the balanced false positive and false negative model not have a very well balanced false positive and false negative error rates?"
I wanted to point out that we select the model that balances these rates on the training data, not the test data.
As such, there isn't a guarantee that the test classifications will also be balanced - and in this case they aren't.
...
![](images/matchProbDotPlot.png){fig-align="center"  width="90%"}


::: {.notes}

Now, it's all well and good to look at the classification accuracy to compare different models.
However, in practice the examiner would instead consider the estimated probability of a match as a similarity score.

The plot on the slide shows the predicted match probabilities for the test data.
Each of thes points represents a single pairwise comparison, and we distinguish those comparisons by whether they are truly matching or non-matching.
We can see on top that the non-match comparisons generally have small match probabilities, which is certainly something we hope for.
Many of the matching comparisons have large match probabilities, but there is a distinct group of comparisons that have very small match probabilities.
This explains the large false negative error rates we saw on the last slide.

One thing we've noticed with the logistic regression model is that the predictred match probabilities tend to be very polarized, meaning the probabilities are either very close to 1 or very close to 0.
...
- Develop free, open source software to implement visual diagnostics & ACES
  ...
  - Explore other optimization criteria
...
## Appendix: Firearm-wise Probabilities {.smaller}

- Specific firearms in the test set tend to have lower associated match probabilities

![](images/matchProbDotPlot_byBarrel.png){fig-align="center" width="70%"}

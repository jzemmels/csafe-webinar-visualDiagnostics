...
In the first row, we can see that the model selected to minimize the overall error rate has a high overall accuracy of 99.4%, although the difference between the true positive and negative rates are different.

In second row, the model selected to balance the true positive and true negative rates has a smaller overall accuracy, yet with closer true positive and negative rates
...
We don't include overall accuracy here because there is currently some debate about how to treat inconclusives when calculating accuracy -- for example, should be counted among the "correct" or "incorrect" classifications or considered at all? We don't have a good answer for this yet.
...
They also considered inconclusives in the study, which may explain the differences in the true negative rates between the model and study.
...
There are about 3000 matching comparisons and 42,0000 non-matching comparisons.
This is not at all unique to cartridge case comparisons, but I wanted to point this out to emphasize that we tend to get better results if we err on the side of making non-matching classifications. 
...
Keep in mind that we selected this model because it balances these rates on the training data, not the test data.
There isn't a guarantee that the test classifications will also be balanced - and in this case they aren't.
...
Many of the matching comparisons have large similarity score, but there is a group of comparisons that have very small similarity score.
In particular, we discovered that matching comparisons between scans fired from the firearm labeled "T" tend to have lower similarity scores.
This might contribute to the low true positive rate we noted on the last slide.
...
Specifically, we introduced a set of diagnostic tools that are useful for explaining the behavior of the ACES algorithm and for comparing cartridge case evidence.
...
We need to devise additional "stress tests" so that we understand the strengths and limitations of the ACES algorithm.

...
## Ames I Study {.smaller .scrollable}
...
The researchers were particularly interested in assessing the true positive and true negative error rates of these examiners
A true positive error occurs when a non-match cartridge case is classified as a match while a true negative error occurs when a matching cartridge case is classified as a non-match

**NEXT**

The results they got back showed that the examiners made very few errors during their examinations.
Out of 3,268 comparisons, examiners only got 26 wrong with a true positive rate of 1.01% and true negative rate of 0.37%.
...
- *Inconclusive*: conclusion when there is some agreement or disagreement in characteristics, but not enough to make a match or non-match conclusion [@AFTE1992]
...
- Train random logistic regression using 21,945 pairwise comparisons from 210 scans
  ...
  - Explore two optimization criteria:
    ...
    - Model that balances true positive and true negative error rates
...
So given a cartridge case pair, this logistic regression model will classify that pair a match or a non-match based on a similarity score.
The process of training a model involves selecting model parameters that optimize some criteria.
For example, we consider two different optimization criteria: one where we select the model that minimizes the overall classification error rate and another that balances the true positive and true negative error rates.
...
We are interested not only in the true positive and true negative error rates, but we also want to consider the predicted probabilities of the matching and non-matching comparisons since this actually what an examiner would use during their examination.
...
In the first row, we can see that the model selected to minimize the overall error rate has a low error rate of 0.59% which consists of a true positive error rate of 0.07% and true negative error rate of 7.66%.
In second row, the model selected to balance the false palse positive and true negative error rates has a larger overall error rate of 2.09%, but this consists of a true positive and true negative error rates that are much closer to each other.
...
For example, we could consider whether a true positive error has worse ethical consequences than a true negative - or vice versa.
...
We see that the first model actually has a lower overall error rate, although this is at the expense of a much larger true negative error rate.
...
For example, you can see this represented in the first model that a very low true positive rate but a large true negative rate.
This model seemingly has a preference for making non-match classifications.

**NEXT**

Finally, some of you may be looking at these results and think "why does the balanced true positive and true negative model not have a very well balanced true positive and true negative error rates?"
...
![](images/matchProbDotPlot_firearmT.png){fig-align="center"  width="70%"}

::: {.notes}

Now, it's all well and good to look at the classification accuracy to compare different models.
However, in practice the examiner would instead consider the  similarity score.
...
Many of the matching comparisons have large similarity score, but there is a distinct group of comparisons that have very small similarity score.
This explains the large true negative error rates we saw on the last slide.

One thing we've noticed with the logistic regression model is that the predictred similarity score tend to be very polarized, meaning the probabilities are either very close to 1 or very close to 0.
